...
public class ActorsMoviesRepository {
    ...
    public List<Movie> getMoviesByActor(String actorName) {
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = connection.prepareStatement(
                     "SELECT movies.id, title, release_date, actors.actor_name" +
                             "FROM movies" +
                             "LEFT OUTER JOIN actors_movies ON movies.id = actors_movies.movie_id" +
                             "LEFT OUTER JOIN actors ON actor_id = actors.id" +
                             "HAVING actors.actor_name = '?'")) {
    ...
    private List<Movie> processListOfMoviesByResultSet(PreparedStatement statement) {
        ...
        try (ResultSet resultSet = statement.executeQuery()) {
            ...
            throw new IllegalStateException("Cannot query: " + statement);
        }
        return movies;
    }
    ...
    public List<Actor> getActorsByMovie(String movieTitle) {
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = connection.prepareStatement(
                     "SELECT * FROM actors JOIN actors_movies ON actors.id = actors_movies.actor_id JOIN movies ON actors_movies.movie_id = movies.id HAVING movies.title = '?';")) {
